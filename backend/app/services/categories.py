...
async def createCategory(data: CategorySchema, db: Prisma):
...
async def getAllCategories(db: Prisma):
    categories = await db.category.find_many()
    return categories

async def getCategory(category_id: str, db: Prisma):
    category = await db.category.find_unique(where={"id": category_id})
    if not category:
        raise HTTPException(status_code=404, detail="Category not found")
    return category

async def updateCategory(category_id: str, data: CategoryUpdateSchema, db: Prisma):
...
async def deleteCategory(category_id: str, db: Prisma):
